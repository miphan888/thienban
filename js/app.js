...
// ─── State ───
const State = {
  theme: localStorage.getItem('thienban_theme') || 'light',
  currentView: 'landing',
  lasoBD: null,
...
function renderLaSo(bd) {
  ...
  var duong = bd.duongLich.ngay+'/'+bd.duongLich.thang+'/'+bd.duongLich.nam;
  var am = bd.amLich.ngay+'/'+bd.amLich.thang+(bd.amLich.isLeap?'(n)':'')+'/'+bd.amLich.nam;
  document.getElementById('lasoMeta').innerHTML =
    ...
    + '<span class="meta-chip gold">'+(bd.banMenhFull||bd.banMenh)+'</span>'
    + '<span class="meta-chip">'+bd.amDuongGT+' '+bd.huongDi+'</span>';
...
function renderTabContent(tab) {
  ...
  } else if (tab === 'luangiai') {
    container.appendChild(buildLuanGiaiView(bd));
  }
}
...
function buildCenterCell(bd) {
  ...
  cell.innerHTML = '<div class="center-logo">☵</div>'
    + '<div class="center-name">' + (bd.ten || '—') + '</div>'
    + '<div class="center-grid">'
    + '<div class="center-item"><label>Dương Lịch</label><span>'
      + bd.duongLich.ngay+'/'+bd.duongLich.thang+'/'+bd.duongLich.nam+'</span></div>'
    + '<div class="center-item"><label>Âm Lịch</label><span>'
      + bd.amLich.ngay+'/'+bd.amLich.thang+'/'+bd.amLich.nam+'</span></div>'
    + '<div class="center-item"><label>Năm</label><span class="gold">'
      + bd.namCanChi+'</span></div>'
    + '<div class="center-item"><label>Ngày</label><span>'
      + (bd.ngayCanChi||'')+'</span></div>'
    + '<div class="center-item"><label>Giờ</label><span>'
      + bd.gioCanChi+'</span></div>'
    + '<div class="center-item"><label>Bản Mệnh</label><span class="gold">'
      + (bd.banMenhFull||bd.banMenh)+'</span></div>'
    + '<div class="center-item"><label>Cục</label><span class="gold">'
      + bd.cucName+'</span></div>'
    + '<div class="center-item"><label>'+bd.amDuongGT+'</label><span>'
      + bd.huongDi+'</span></div>'
    + '<div class="center-item"><label>Cung Mệnh</label><span>'
      + bd.cungMenh_diachi+'</span></div>'
    + '<div class="center-item"><label>Cung Thân</label><span>'
      + bd.cungThan_diachi+'</span></div>'
    + '<div class="center-item"><label>Mệnh Chủ</label><span>'
      + (bd.menhChu||'')+'</span></div>'
    + '<div class="center-item"><label>Thân Chủ</label><span>'
      + (bd.thanChu||'')+'</span></div>'
    + '</div>';
  return cell;
}
...
function buildDaiVanView(bd) {
  ...
  div.style.cssText = 'max-width:900px;margin:0 auto;';

  var title = document.createElement('h3');
  title.style.cssText = 'font-family:var(--font-display);font-size:1.4rem;margin-bottom:1.5rem;';
  title.textContent = 'Đại Vận — ' + (bd.thuan ? 'Thuận hành' : 'Nghịch hành');
  div.appendChild(title);

  var DIACHI = window.TuViAlgorithm.DIACHI;

  // Lấy luận giải
  var lgDaiHan = [];
  if (window.LuanGiai) {
    try {
      var lg = window.LuanGiai.luanGiaiTongHop(bd);
      lgDaiHan = lg.daiHan || [];
    } catch(e) { console.warn('Lỗi luận giải:', e); }
  }

  var grid = document.createElement('div');
  grid.style.cssText = 'display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:12px;';

  for (var i = 0; i < bd.daiVans.length; i++) {
    var dv = bd.daiVans[i];
    var cungData = bd.cungs[dv.cung];
    if (!cungData) continue;
    var stars = cungData.stars || [];

    // Chính tinh
    var chinhText = [];
    for (var ci = 0; ci < stars.length; ci++) {
      if (stars[ci].type === 'chinh') {
        var t = stars[ci].name;
        if (stars[ci].mieuHam) t += ' ' + stars[ci].mieuHam;
        chinhText.push(t);
      }
    }
    ...
    // Đối cung
    var doiText = '';
    if (chinhText.length === 0) {
      var doiIdx = (dv.cung + 6) % 12;
      var doiStars = bd.cungs[doiIdx] && bd.cungs[doiIdx].stars ? bd.cungs[doiIdx].stars : [];
      var doiChinh = [];
      for (var di2 = 0; di2 < doiStars.length; di2++) {
        if (doiStars[di2].type === 'chinh') doiChinh.push(doiStars[di2].name);
      }
      doiText = doiChinh.length > 0
        ? 'Đối cung ' + DIACHI[doiIdx] + ': ' + doiChinh.join(', ') + ' chiếu'
        : 'Vô chính diệu, đối cung cũng trống';
    }

    // Luận giải
    var luanLines = (lgDaiHan[i] && lgDaiHan[i].nhanDinh) ? lgDaiHan[i].nhanDinh : [];

    // Tổng đánh giá
    var tongKet = '';
    var badgeClass = 'dv-binh';
    for (var tl = luanLines.length - 1; tl >= 0; tl--) {
      if (luanLines[tl].indexOf('📊') >= 0) {
        tongKet = luanLines[tl].replace('📊 ', '');
        if (tongKet.indexOf('ĐẠI CÁT') >= 0) badgeClass = 'dv-dacat';
        else if (tongKet.indexOf('TIỂU CÁT') >= 0) badgeClass = 'dv-tieucat';
        else if (tongKet.indexOf('CÁT') >= 0) badgeClass = 'dv-cat';
        else if (tongKet.indexOf('ĐẠI HUNG') >= 0) badgeClass = 'dv-dahung';
        else if (tongKet.indexOf('HUNG') >= 0) badgeClass = 'dv-hung';
        else badgeClass = 'dv-binh';
        tongKet = tongKet.split(' — ')[0]; // chỉ lấy "ĐẠI CÁT"
        break;
      }
    }
    if (!tongKet) tongKet = 'BÌNH';

    // Build card
    var card = document.createElement('div');
    card.className = 'dv-card';

    var headerHTML = '<div class="dv-card-header">'
      + '<div><div class="dv-tuoi">Tuổi ' + dv.tuoiDau + ' – ' + dv.tuoiCuoi + '</div>'
      + '<div class="dv-cung">' + DIACHI[dv.cung] + '</div></div>'
      + '<span class="dv-badge ' + badgeClass + '">' + tongKet + '</span></div>';

    var chucHTML = '<div class="dv-chuc">' + (cungData.cungChuc || '') + '</div>';

    var chinhHTML = chinhText.length > 0
      ? '<div class="dv-chinh">' + chinhText.join(' • ') + '</div>'
      : '<div class="dv-chinh" style="color:var(--text-tertiary)">(vô chính diệu)</div>';

    var doiHTML = doiText ? '<div class="dv-doi">' + doiText + '</div>' : '';

    // Chi tiết luận giải (ẩn, bấm mở)
    var detailsHTML = '<div class="dv-details">';
    for (var li = 0; li < luanLines.length; li++) {
      if (luanLines[li].indexOf('📊') >= 0) continue; // bỏ dòng tổng kết (đã hiện badge)
      detailsHTML += '<div class="dv-line">' + luanLines[li] + '</div>';
    }
    detailsHTML += '</div>';

    var toggleHTML = '<div class="dv-toggle">▼ Xem chi tiết</div>';

    card.innerHTML = headerHTML + chucHTML + chinhHTML + doiHTML + detailsHTML + toggleHTML;

    // Toggle sự kiện
    (function(cardEl) {
      var toggle = cardEl.querySelector('.dv-toggle');
      if (toggle) {
        toggle.addEventListener('click', function() {
          cardEl.classList.toggle('open');
          toggle.textContent = cardEl.classList.contains('open') ? '▲ Thu gọn' : '▼ Xem chi tiết';
        });
      }
    })(card);
...
function buildThongTinView(bd) {
  ...
  var sections = [
    { title: 'Thông tin cơ bản', items: [
      ...
      ['Ngày Can Chi', bd.ngayCanChi || '—'],
      ['Giờ sinh', bd.gioSinh + ' (' + bd.gioCanChi + ')'],
    ]},
    { title: 'Cung Mệnh & Vận', items: [
      ['Bản Mệnh (Nạp Âm)', bd.banMenhFull || bd.banMenh],
      ['Cục số', bd.cucName],
      ['Sao chủ Cục', bd.cucChu || '—'],
      ['Cung Mệnh', bd.cungMenh_diachi],
      ['Mệnh Chủ', bd.menhChu || '—'],
      ['Cung Thân', bd.cungThan_diachi],
      ['Thân Chủ', bd.thanChu || '—'],
      ['Thân cư', bd.thanCuCung || '—'],
      [bd.amDuongGT || '', bd.huongDi || ''],
      ['Mệnh Cục', bd.menhCucQH || '—'],
      ['Khởi đại vận', 'Tuổi '+(bd.daiVans[0]?bd.daiVans[0].tuoiDau:'—')+' ('+bd.huongDi+')'],
    ]},
    { title: 'Tứ Hóa', items: Object.entries(bd.tuHoa).map(function(e){return[e[1].join('+'),e[0]];}) },
  ];
  ...
  sections.forEach(function(sec) {
    ...
    sec.items.forEach(function(item) {
      ...
      row.innerHTML = '<span style="color:var(--text-secondary)">'+item[0]+'</span>'
        + '<span style="color:var(--text-primary);font-weight:500;text-align:right;max-width:60%">'+item[1]+'</span>';
...
// ─── Luận Giải View ───
function buildLuanGiaiView(bd) {
  const div = document.createElement('div');
  div.className = 'animate-in';
  div.style.cssText = 'max-width:760px;margin:0 auto;display:flex;flex-direction:column;gap:20px;';

  if (!window.LuanGiai) {
    div.innerHTML = '<p style="color:var(--text-tertiary)">Module luận giải chưa được tải.</p>';
    return div;
  }

  let lg;
  try {
    lg = window.LuanGiai.luanGiaiTongHop(bd);
  } catch(e) {
    div.innerHTML = '<p style="color:#cc3333">Lỗi luận giải: '+e.message+'</p>';
    return div;
  }

  // Tổng quan
  if (lg.tongQuan.length) div.appendChild(buildLGCard('☯ Tổng Quan Mệnh', lg.tongQuan));

  // Cách cục
  if (lg.cachCuc.length) {
    const items = lg.cachCuc.map(cc => {
            const icon = cc.muc==='quý'?'🌟':cc.muc==='phú'?'💰':'⚠️';
      return icon+' '+cc.ten+': '+cc.luan;
    });
    div.appendChild(buildLGCard('🏛 Cách Cục', items));
  }

  // Tuần Triệt
  if (lg.tuanTriet.length) div.appendChild(buildLGCard('⊘ Tuần Triệt', lg.tuanTriet));

  // 12 Cung
  const order = ['Mệnh','Phu Thê','Tài Bạch','Quan Lộc','Phúc Đức',
    'Tử Tức','Huynh Đệ','Thiên Di','Tật Ách','Điền Trạch','Nô Bộc','Phụ Mẫu'];
  for (const tc of order) {
    if (lg.luanCungs[tc] && lg.luanCungs[tc].length) {
      div.appendChild(buildLGCard('📍 '+tc, lg.luanCungs[tc]));
    }
  }
  ...
  if (lg.daiHan.length) {
    const items = [];
    for (const dh of lg.daiHan) {
      items.push(`【${dh.tuoi} · ${dh.cung}】`);
      for (const nd of dh.nhanDinh) items.push('  '+nd);
    }
    div.appendChild(buildLGCard('⏳ Đại Hạn', items));
  }

  return div;
}

function buildLGCard(title, items) {
  const card = document.createElement('div');
  card.style.cssText = 'background:var(--bg-surface);border:1px solid var(--border-subtle);border-radius:12px;overflow:hidden;';

  const h = document.createElement('div');
  h.style.cssText = 'padding:12px 16px;border-bottom:1px solid var(--border-subtle);font-size:1rem;font-weight:600;color:var(--text-gold);font-family:var(--font-display);';
  h.textContent = title;
  card.appendChild(h);

  const b = document.createElement('div');
  b.style.cssText = 'padding:12px 16px;';
  for (const item of items) {
    const p = document.createElement('p');
    p.style.cssText = 'font-size:0.875rem;color:var(--text-secondary);line-height:1.7;padding:5px 0;border-bottom:1px solid var(--border-subtle);';
    p.textContent = item;
    b.appendChild(p);
  }
  card.appendChild(b);
